...
def run_all(start='2018-01-01', prefer='yahoo', nav=100000.0, run_walk_forward=False):
    ...
    latest_regime_v5 = str(macro_summary.get('risk_regime') or overlay['overlay_regime'].iloc[-1])
    research_note = portfolio_brief(weights_overlay, exposures, regime=latest_regime_v5)

    # Safe formatting: macro_summary may contain None when macro data is missing.
    risk_regime = macro_summary.get('risk_regime') or latest_regime_v5
    recession_prob_6m = macro_summary.get('recession_probability_6m')
    equity_risk_score = macro_summary.get('equity_risk_score')
    credit_stress_score = macro_summary.get('credit_stress_score')

    recession_prob_6m = 0.0 if recession_prob_6m is None or pd.isna(recession_prob_6m) else float(recession_prob_6m)
    equity_risk_score = 0.0 if equity_risk_score is None or pd.isna(equity_risk_score) else float(equity_risk_score)
    credit_stress_score = 0.0 if credit_stress_score is None or pd.isna(credit_stress_score) else float(credit_stress_score)

    research_note += (
        f"\n\nV5.5 Macro-Credit View: regime={risk_regime}, "
        f"recession probability 6M={recession_prob_6m:.1%}, "
        f"equity risk score={equity_risk_score:.1f}, "
        f"credit stress score={credit_stress_score:.1f}."
    )
